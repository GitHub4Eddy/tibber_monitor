...
function QuickApp:getValues() -- Get the values from json file 
  ...
  for hour in pairs(jsonTable.data.viewer.homes[homeNr].currentSubscription.priceInfo.tomorrow or {}) do -- Insert all tomorrow prices to table jsonPrices
    if 24-tonumber(runhour)+hour <= 12 then 
      table.insert(jsonPrices,{hour = tostring(hour-1)..":00", total = jsonTable.data.viewer.homes[homeNr].currentSubscription.priceInfo.tomorrow[hour].total, percentage = string.format("%.2f",((jsonTable.data.viewer.homes[homeNr].currentSubscription.priceInfo.today[hour].total-data.tPrice)/data.tPrice)*100), level = "(" ..jsonTable.data.viewer.homes[homeNr].currentSubscription.priceInfo.tomorrow[hour].level:gsub("_", " ")  ..")", startsAt = jsonTable.data.viewer.homes[homeNr].currentSubscription.priceInfo.tomorrow[hour].startsAt})
    end
  end
